...
{ 
    ...
    public int m_index = 0, m_preIndex = 0;
    ...
    public virtual void OnEndDrag(PointerEventData eventData)
    {
        ...
        this.m_IsNormalizing = true;
        var dir = eventData.position.x - eventData.pressPosition.x;
        var len = Mathf.Abs(dir);
        //当滑动量大于8分之一个屏时，需要翻页到下一个，而不总是通过找最近的正确位置
        if (len > Screen.width / 8f && len < Screen.width / 2f)
        {
            m_CurrentPos = ComplementDelta(CalcCorrectDeltaPos());
        }
        else
        {
    ...
    /// <returns></returns>
    private Vector2 ComplementDelta(Vector2 delta)
    {
        int dir = 1;
        if (MoveAxis == Axis.Horizontal)
        {
            if (delta.x < 0)
                dir = -1;
            return new Vector2(-dir * (CellSize.x + Spacing.x - dir * delta.x), delta.y);
        }
        else
        {
            if (delta.y < 0)
                dir = -1;
            return new Vector2(delta.x, -dir * (CellSize.y + Spacing.y - dir * delta.y));
        }
    }
